...
func (s *Service) CreateCorrelation(ctx context.Context, cmd *datasources.CreateCorrelationCommand) error {
	return s.SQLStore.InTransaction(ctx, func(ctx context.Context) error {
		...
		ds, err := s.getEditableCorrelationSource(ctx, cmd.OrgID, cmd.SourceUID)
		if err != nil {
			return err
		}
...
func (s *Service) getEditableCorrelationSource(ctx context.Context, orgId int64, uid string) (*datasources.DataSource, error) {
	query := &datasources.GetDataSourceQuery{
		OrgId: orgId,
		Uid:   uid,
	}
